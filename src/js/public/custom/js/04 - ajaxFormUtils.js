...
function fillForm($form, data, soloData, resetearData = true) {
    if (resetearData) {
        if (soloData) {
            resetFormData($form);
        } else {
...
function habilitarForm($form) {
    $form.find('input, select, textarea').prop('disabled', false);
}

function aplicarMensajeError($form) {
...
function showConfirmMessage(message, callbackConfirm, callbackCancel) {
    Swal.fire({
        title: "¿Está seguro?",
        text: message,
        type: "warning",
...
function ajaxPostCall(url, data, messageSuccess, successCallback) {
    $.ajax({
        data: data,
        url: url,
        type: 'POST',
...
function ajaxGetCall(url, data, successCallback) {
    $.ajax({
        data: data,
        url: url,
        type: 'GET',
