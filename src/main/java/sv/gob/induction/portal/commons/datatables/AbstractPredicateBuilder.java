...
import sv.gob.induction.portal.commons.datatables.mapping.Column;
import sv.gob.induction.portal.commons.datatables.mapping.DataTablesInput;
import sv.gob.induction.portal.commons.datatables.mapping.Order;
...
abstract class AbstractPredicateBuilder<T> {
    ...
    AbstractPredicateBuilder(DataTablesInput input) {
        ...
        if (this.hasGlobalFilter) {
            tree = new Node<>(null, new GlobalFilter(input.getSearch()));
        } else {
            tree = new Node<>(null);
        }
        initTree(input);
    ...
    private void addChild(Node<Filter> parent, int index, String[] names, Search search) {
        ...
        if (isLast) {
            ...
            parent.addChild(new Node<>(names[index], hasColumnFilter ? new ColumnFilter(search) : null));
        } else {
